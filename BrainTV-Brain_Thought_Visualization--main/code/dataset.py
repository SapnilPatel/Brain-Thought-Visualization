...
def img_norm(img):
    if img.shape[-1] == 3:
        img = rearrange(img, 'h w c -> c h w') # TODO: explore what are the h,w,& c? -> hwc stands for height, width, and channel, and it is a common way to represent the dimensions of an image in deep learning.
...
class hcp_dataset(Dataset):
    def __init__(self, path='../data/HCP/npz', roi='VC', patch_size=16, transform=identity, aug_times=2, 
        ...
        if include_hcp:
            for c, sub in enumerate(os.listdir(path)):
                ...
                voxels = np.concatenate([npz['V1'],npz['V2']], axis=-1) if roi == 'VC' else npz[roi] # 1200, num_voxels
    ...
    # __getitem__ used to get only single image from specific index.
    ...
    def __getitem__(self, index):
        ...
        if self.return_image_class_info:
            ...
            return {'fmri': self.fmri_transform(fmri), 'image': self.image_transform(img),
                    'image_class': img_class, 'image_class_name': img_class_name, 'naive_label':naive_label} # TODO: explore naive_label keyword
        else:
            return {'fmri': self.fmri_transform(fmri), 'image': self.image_transform(img)}
...
# TODO: in which form stimuli is present in the dataset?
...
# The function list_get_all_index() you provided is a concise and efficient way to find all indices of a value in a list in Python.
# It works by using a list comprehension to iterate over the list and return the indices of the elements that are equal to the given value.
